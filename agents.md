...
- **Active**:
  ...
  - **Proxmox VE node `pve`** (`10.30.1.55`) — hosts standalone cloud-init Ubuntu VMs (IPs `10.30.1.56-.59`) defined by Terraform in `proxmox/`. The `hermes` VM (`10.30.1.57`) also runs the media stack (Gluetun + qBittorrent + Plex) deployed from `hermes/`.
  - QNAP NAS (`10.30.1.20`)
...
## 6c. hermes media stack (`hermes/`)

- **`hermes/` is the source of truth** for the Docker media stack on the hermes VM
  (`ubuntu@10.30.1.57`): Gluetun (NordVPN WireGuard) + qBittorrent + Plex. Deploy with
  `./hermes/deploy-to-hermes.sh` (idempotent: bootstraps Docker, rsyncs, generates the
  remote `.env` from `secrets.yaml`, `compose up`). Never `docker compose` by hand on
...
- **Tailscale × Docker gotcha (will bite again)**: hermes runs a manually-installed
  ...
  node on hermes — it would capture gluetun's tunnel. Details: `hermes/README.md`.
...
- **Storage**: 1TB USB HDD whole-disk passthrough at `/mnt/data` (`nofail`) holds
  ...
  `/home/ubuntu/appdata/`. The Terraform `data_disk_path` attach requires a one-time
  root `qm set` on pve (the API token cannot pass through devices); terraform then
